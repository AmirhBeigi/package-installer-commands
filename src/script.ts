...
sidebarEl?.insertAdjacentHTML('afterbegin', `<div id="package-box"></div>`);
...
const cleanUp = () => {
    document
        .querySelector(
            '#top > div.fdbf4038.w-third-l.mt3.w-100.ph3.ph4-m.pv3.pv0-l > div.d767adf4.lh-copy.truncate.ph0.mb3.black-80.b5be2af6.f6.flex.flex-row'
        )
        ?.remove();
    document
...
function generateCommand(
    manager: Manager,
    options: { isDev: boolean; isOptional: boolean } = {
...
const renderInstallPackageBox = (manager: Manager) => {
    const command = generateCommand(manager);

    return `<div id="${manager}-box" class="d767adf4 lh-copy truncate ph0 mb3 black-80 b5be2af6 f6 flex flex-row">
